    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:flex items-end justify-between gap-4 relative z-10">
            {funnelStages.map((stage, index) => (
              <div
                key={stage.id}
                className="relative flex flex-col items-center funnel-step"
                data-step-id={stage.id}
                style={{ flex: 1 }}
              >
                {/* KPI Card - positioned above step */}
                <div className="funnel-kpi w-full max-w-xs mb-4">
                  <div className="double-bezel p-4 sm:p-5 text-center group relative">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      <span className="text-xs font-medium uppercase tracking-[0.1em] text-aima-purple-light">
                        {stage.label}
                      </span>
                      <div className="flex h-1.5 w-1.5 rounded-full bg-aima-purple/50 animate-pulse" />
                    </div>
                    <div className="text-aima-gradient text-3xl sm:text-4xl font-bold tracking-tight">
                      {stage.kpi.value}
                      <span className="text-lg font-normal text-white/60">{stage.kpi.label}</span>
                    </div>
                    <p className="mt-1 text-xs text-white/50">{stage.kpi.detail}</p>
                  </div>
                </div>

                {/* Step Circle */}
                <div className="relative flex h-24 w-24 items-center justify-center rounded-full border border-white/10 bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f] mb-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
                ...
                <div className="w-full max-w-xs text-center">
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex items-center justify-center gap-2 text-white/60 bg-white/5 rounded-lg px-3 py-2">
                      <IconComponents.Bot size={14} className="text-aima-purple-light" />
                      <span>IA: {stage.aiAction}</span>
                    </div>
                    <div className="flex items-center justify-center gap-2 text-white/60 bg-white/5 rounded-lg px-3 py-2">
                      <IconComponents.User size={14} className="text-emerald-400" />
                ...
                {index < funnelStages.length - 1 && (
                  <div className="funnel-connector absolute top-1/2 right-[-50%] w-full h-px bg-gradient-to-r from-transparent via-aima-purple/40 to-transparent" />
                )}
              </div>
            ))}
          </div>
